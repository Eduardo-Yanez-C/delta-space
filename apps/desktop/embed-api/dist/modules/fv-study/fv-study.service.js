...
exports.FvStudyService = exports.MOUNTING_TYPE_VALUES = exports.GENERATION_SOURCE_VALUES = void 0;
// @ts-nocheck
...
const suggestedItemsMatching = __importStar(require("./suggested-items-matching"));
const product_quote_display_name_1 = require("../../common/product-quote-display-name");
const system_scenario_1 = require("./domain/system-scenario");
...
exports.GENERATION_SOURCE_VALUES = ["INTERNAL", "EXPLORADOR_SOLAR", "MANUAL", "EXTERNAL"];
...
exports.MOUNTING_TYPE_VALUES = ["TECHO", "SUELO", "INCLINADO_FIJO", "SEGUIMIENTO", "OTRO"];
...
function validateAndNormalizeMonths(months) {
    if (!Array.isArray(months) || months.length !== 12) {
        throw new common_1.BadRequestException("Debe enviar exactamente 12 meses (monthIndex 1 a 12 con consumptionKwh).");
    }
    const byIndex = new Map();
    for (const m of months) {
        const idx = Number(m.monthIndex);
        if (!Number.isInteger(idx) || idx < 1 || idx > 12) {
            throw new common_1.BadRequestException(`monthIndex debe ser entero entre 1 y 12; recibido: ${m.monthIndex}`);
        }
        if (byIndex.has(idx)) {
            throw new common_1.BadRequestException(`monthIndex duplicado: ${idx}`);
        }
        const consumption = Number(m.consumptionKwh);
        if (typeof consumption !== "number" || consumption < 0 || !Number.isFinite(consumption)) {
            throw new common_1.BadRequestException(`consumptionKwh inválido para mes ${idx}`);
        }
        byIndex.set(idx, consumption);
    }
    for (let i = 1; i <= 12; i++) {
        if (!byIndex.has(i)) {
            throw new common_1.BadRequestException(`Falta monthIndex ${i}. Debe haber exactamente un registro por mes (1 a 12).`);
        }
    }
...
function validateAndNormalizeMonthsForManual(months) {
    if (!Array.isArray(months) || months.length !== 12) {
        throw new common_1.BadRequestException("Para generación manual debe enviar exactamente 12 meses (monthIndex 1 a 12) con consumptionKwh y generationKwh.");
    }
    const byIndex = new Map();
    for (const m of months) {
        const idx = Number(m.monthIndex);
        ...
        const consumption = Number(m.consumptionKwh);
        if (typeof consumption !== "number" || consumption < 0 || !Number.isFinite(consumption)) {
            throw new common_1.BadRequestException(`consumptionKwh debe ser >= 0 para mes ${idx}.`);
        }
        const gen = m.generationKwh;
        if (gen === undefined || gen === null) {
            throw new common_1.BadRequestException(`Para generación manual, generationKwh es obligatorio en cada mes. Falta en mes ${idx}.`);
        }
        const generation = Number(gen);
        if (typeof generation !== "number" || generation < 0 || !Number.isFinite(generation)) {
            throw new common_1.BadRequestException(`generationKwh debe ser >= 0 para mes ${idx}.`);
        }
    ...
    if (fixedPanels > 0) {
        ...
        if (params.potenciaSistemaKwp != null && params.potenciaSistemaKwp > 0) {
            ...
            if (generacionAnualPorKwp <= 0)
                throw new common_1.BadRequestException("Parámetros de generación inválidos (HSP/PR).");
...
let FvStudyService = class FvStudyService {
    ...
    async update(id, dto, currentUser) {
        ...
        if (study.status === "ARCHIVADO") {
            throw new common_1.BadRequestException("No se puede editar un estudio archivado.");
        }
        ...
        if (dto.status != null && !VALID_STATUSES.includes(dto.status)) {
            throw new common_1.BadRequestException(`status debe ser uno de: ${VALID_STATUSES.join(", ")}`);
        }
        this.validateSolarResource(dto);
        ...
        if (dto.utilityGridAvailable !== undefined) {
            if (typeof dto.utilityGridAvailable !== "boolean") {
                throw new common_1.BadRequestException("utilityGridAvailable debe ser boolean.");
            }
        ...
        if (dto.gridExportEnabled !== undefined) {
            if (typeof dto.gridExportEnabled !== "boolean") {
                throw new common_1.BadRequestException("gridExportEnabled debe ser boolean.");
            }
        ...
        const nextScenarioInput = {
            systemType: updateData.systemType !== undefined ? updateData.systemType : study.systemType ?? "ON_GRID",
            utilityGridAvailable: updateData.utilityGridAvailable !== undefined
                ? updateData.utilityGridAvailable
                : study.utilityGridAvailable,
            gridExportEnabled: updateData.gridExportEnabled !== undefined
    ...
    async remove(id, currentUser) {
        ...
        try {
            ...
            if (e instanceof client_1.Prisma.PrismaClientKnownRequestError && e.code === "P2003") {
                throw new common_1.BadRequestException("No se puede eliminar: el estudio sigue referenciado de forma que impide borrarlo.");
            }
            throw e;
        }
        return { deleted: true };
            ...
            if (createWithSuggestedItems) {
                ...
                const buildLineData = (result, manual) => {
                    if (result.fromCatalog) {
                        ...
                        const displayName = (0, product_quote_display_name_1.commercialNameForQuoteLine)(result.product);
                        return {
    ...
    async assertCanWrite(study, currentUser) {
        ...
        if (roles.includes(roleConstants.ROLE_VENTAS_LEGACY) ||
            ...
            roles.includes("INGENIERIA"))
            return;
        throw new common_1.ForbiddenException("Sin permiso para modificar este estudio FV.");
    }
    validateMonthIndex(value, field) {
        const n = Number(value);
        if (!Number.isInteger(n) || n < 1 || n > 12) {
            throw new common_1.BadRequestException(`${field} debe ser un entero entre 1 y 12; recibido: ${value}`);
        }
    }
    validateConnectionType(connectionType) {
        if (!VALID_CONNECTION_TYPES.includes(connectionType)) {
            throw new common_1.BadRequestException(`connectionType debe ser MONOFASICO o TRIFASICO; recibido: ${connectionType}`);
        }
    }
    validateSystemType(systemType) {
        if (!VALID_SYSTEM_TYPES.includes(systemType)) {
            throw new common_1.BadRequestException(`systemType debe ser ON_GRID, OFF_GRID o HYBRID; recibido: ${systemType}`);
        }
    }
    validateFvStudyGridFlagsOnCreate(dto) {
        if (dto.utilityGridAvailable === undefined || dto.gridExportEnabled === undefined) {
            throw new common_1.BadRequestException("En la creación son obligatorios utilityGridAvailable y gridExportEnabled (boolean).");
        }
        if (typeof dto.utilityGridAvailable !== "boolean" || typeof dto.gridExportEnabled !== "boolean") {
            throw new common_1.BadRequestException("utilityGridAvailable y gridExportEnabled deben ser boolean.");
        }
    }
    assertFvSystemConfig(input) {
        const errs = (0, system_scenario_1.collectFvSystemConfigErrors)(input);
        if (errs.length === 0)
            return;
        if (errs.length === 1) {
            throw new common_1.BadRequestException(errs[0]);
        }
    ...
    validateSolarResource(dto) {
        if (dto.generationSource != null && !ACCEPTED_GENERATION_SOURCES.includes(dto.generationSource)) {
            throw new common_1.BadRequestException(`generationSource debe ser INTERNAL, MANUAL o EXPLORADOR_SOLAR. Valor no implementado: ${dto.generationSource}.`);
        }
        if (dto.latitude != null && (typeof dto.latitude !== "number" || dto.latitude < -90 || dto.latitude > 90)) {
            throw new common_1.BadRequestException("latitude debe ser un número entre -90 y 90.");
        }
        if (dto.longitude != null && (typeof dto.longitude !== "number" || dto.longitude < -180 || dto.longitude > 180)) {
            throw new common_1.BadRequestException("longitude debe ser un número entre -180 y 180.");
        }
        if (dto.tiltDegrees != null && (typeof dto.tiltDegrees !== "number" || dto.tiltDegrees < 0 || dto.tiltDegrees > 90)) {
            throw new common_1.BadRequestException("tiltDegrees debe ser un número entre 0 y 90.");
        }
        if (dto.azimuthDegrees != null && (typeof dto.azimuthDegrees !== "number" || dto.azimuthDegrees < 0 || dto.azimuthDegrees > 360)) {
            throw new common_1.BadRequestException("azimuthDegrees debe ser un número entre 0 y 360.");
        }
        if (dto.mountingType != null && !exports.MOUNTING_TYPE_VALUES.includes(dto.mountingType)) {
            throw new common_1.BadRequestException(`mountingType debe ser uno de: ${exports.MOUNTING_TYPE_VALUES.join(", ")}.`);
        }
        if (dto.solarResourceMetadata != null && typeof dto.solarResourceMetadata !== "string") {
            throw new common_1.BadRequestException("solarResourceMetadata debe ser un string.");
        }
    }
    ...
    toResponse(row) {
        return {
            ...
            systemScenario: (0, system_scenario_1.getSystemScenarioOrNull)({
...
exports.FvStudyService = FvStudyService;
exports.FvStudyService = FvStudyService = __decorate([
