"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
var __param = (this && this.__param) || function (paramIndex, decorator) {
    return function (target, key) { decorator(target, key, paramIndex); }
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.InstallationsController = void 0;
const common_1 = require("@nestjs/common");
const jwt_auth_guard_1 = require("../auth/jwt-auth.guard");
const roles_decorator_1 = require("../auth/decorators/roles.decorator");
const roles_guard_1 = require("../auth/roles.guard");
const activate_installation_dto_1 = require("./dto/activate-installation.dto");
const revoke_installation_dto_1 = require("./dto/revoke-installation.dto");
const validate_installation_dto_1 = require("./dto/validate-installation.dto");
const installations_service_1 = require("./installations.service");
let InstallationsController = class InstallationsController {
    constructor(installationsService) {
        this.installationsService = installationsService;
    }
    async activate(dto) {
        return this.installationsService.activate(dto);
    }
    async validate(dto) {
        return this.installationsService.validate(dto.installationId, dto.installationToken);
    }
    findAll() {
        return this.installationsService.findAll();
    }
    revoke(id, dto) {
        return this.installationsService.revoke(id, dto?.note);
    }
};
exports.InstallationsController = InstallationsController;
__decorate([
    (0, common_1.Post)("activate"),
    __param(0, (0, common_1.Body)()),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [activate_installation_dto_1.ActivateInstallationDto]),
    __metadata("design:returntype", Promise)
], InstallationsController.prototype, "activate", null);
__decorate([
    (0, common_1.Post)("validate"),
    __param(0, (0, common_1.Body)()),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [validate_installation_dto_1.ValidateInstallationDto]),
    __metadata("design:returntype", Promise)
], InstallationsController.prototype, "validate", null);
__decorate([
    (0, common_1.Get)(),
    (0, common_1.UseGuards)(jwt_auth_guard_1.JwtAuthGuard, roles_guard_1.RolesGuard),
    (0, roles_decorator_1.Roles)("ADMIN_DEV", "ADMIN"),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", []),
    __metadata("design:returntype", void 0)
], InstallationsController.prototype, "findAll", null);
__decorate([
    (0, common_1.Patch)(":id/revoke"),
    (0, common_1.UseGuards)(jwt_auth_guard_1.JwtAuthGuard, roles_guard_1.RolesGuard),
    (0, roles_decorator_1.Roles)("ADMIN_DEV", "ADMIN"),
    __param(0, (0, common_1.Param)("id")),
    __param(1, (0, common_1.Body)()),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [String, revoke_installation_dto_1.RevokeInstallationDto]),
    __metadata("design:returntype", void 0)
], InstallationsController.prototype, "revoke", null);
exports.InstallationsController = InstallationsController = __decorate([
    (0, common_1.Controller)("installations"),
    __metadata("design:paramtypes", [installations_service_1.InstallationsService])
], InstallationsController);
