...
/** Detecta delimitador: tabulador preferido; si casi no hay tabs, usa `;` o `,`. */
...
/**
 * Primera fila = cabeceras. Columnas reconocidas por nombre flexible (ES/EN).
 * Fechas: YYYY-MM-DD o DD/MM/AAAA (o DD-MM-AAAA).
 */
export function parseScheduleDelimitedText(text: string, maxRows = 350): { rows: ScheduleImportRow[]; warnings: string[] } {
  const warnings: string[] = [];
  const lines = splitLines(text.trim());
  if (lines.length < 2) {
    throw new BadRequestException("El archivo debe tener cabecera y al menos una fila de datos.");
  }
  ...
  if (nameI < 0) {
    throw new BadRequestException(
      "No se encontró columna de nombre de tarea. Use una cabecera como: nombre, tarea, name o title.",
    );
  }
  if (startI < 0 || endI < 0) {
    throw new BadRequestException(
      "Faltan columnas de fechas. Use inicio/fin, start/end o startDate/endDate (YYYY-MM-DD o DD/MM/AAAA).",
    );
  }
  ...
  for (let li = 1; li < lines.length; li++) {
    ...
    if (rows.length >= maxRows) {
      warnings.push(`Se importaron como máximo ${maxRows} filas; el resto se ignoró.`);
      break;
    }
  }
  if (rows.length === 0) {
    throw new BadRequestException("No quedó ninguna fila válida tras el análisis del archivo.");
  }
