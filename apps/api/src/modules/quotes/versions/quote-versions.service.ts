...
export const ADICIONALES_MAIN_ITEM_NAME = "Adicionales";
function toNum(d) {
    if (d == null)
        return 0;
...
export class QuoteVersionsService {
    ...
    async create(quoteId, dto, createdById, currentUser) {
        ...
        if (dto.sourceVersionId) {
            ...
            if (!source) {
                throw new BadRequestException("Versión origen no encontrada");
            }
    ...
    async refreshVersionFromStudy(quoteId, versionId, currentUser) {
        ...
        if (quote.status !== "BORRADOR") {
            throw new BadRequestException("Solo se pueden actualizar ítems en cotizaciones en estado BORRADOR");
        }
        if (!quote.sourceFvStudyId) {
            throw new BadRequestException("La cotización no está vinculada a un estudio FV");
        }
        ...
        if (version.status !== "BORRADOR") {
            throw new BadRequestException("Solo se puede actualizar desde estudio cuando la versión está en BORRADOR");
        }
        const study = await this.prisma.fvStudy.findUnique({
            ...
            for (const m of mainItems) {
                for (const l of m.lines ?? []) {
                    ...
                    await tx.quoteItemLine.update({
                        where: { id: l.id },
                        data: {
                            marginPercentSnapshot: lineMarginPercent != null ? new Prisma.Decimal(lineMarginPercent) : null,
                        },
                    });
                }
            }
    ...
    async acceptAddonSuggestion(quoteId, versionId, suggestionId, currentUser) {
        ...
        if (suggestion.status !== "PENDING") {
            throw new BadRequestException("La sugerencia ya fue aceptada o rechazada");
        }
    ...
    async acceptAddonSuggestionHierarchical(versionId, suggestionId, suggestion) {
        ...
        const line = await this.prisma.$transaction(async (tx) => {
            let mainItem = await tx.quoteMainItem.findFirst({
                where: { quoteVersionId: versionId, name: ADICIONALES_MAIN_ITEM_NAME },
            });
            if (!mainItem) {
                ...
                mainItem = await tx.quoteMainItem.create({
                    data: {
                        ...
                        name: ADICIONALES_MAIN_ITEM_NAME,
                        description: null,
