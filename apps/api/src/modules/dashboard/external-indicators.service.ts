...
const MINDICADOR_HEADERS: Record<string, string> = {
    Accept: "application/json",
    "User-Agent": "CotizadorFV/1.0 (https://mindicador.cl; datos Banco Central Chile)",
};

type MindicadorSeriePoint = { fecha?: string; valor?: number | string };
type MindicadorBlock = {
    valor?: number | string;
    fecha?: string;
...
type IndicatorKey = "dolar" | "uf" | "ipc";

export type ExternalIndicatorSeriesPoint = { fecha: string; valor: number };

export type ExternalIndicatorsSnapshot = {
    dolar: { value: number | null; fecha: string | null; unidad: string | null; error?: boolean };
    uf: { value: number | null; fecha: string | null; unidad: string | null; error?: boolean };
    ipc: { value: number | null; fecha: string | null; unidad: string | null; error?: boolean };
    updatedAt: string | null;
    source: string;
    error?: string;
};

export type ExternalIndicatorsSeriesPayload = {
...
function parseSerie(serie: unknown): ExternalIndicatorSeriesPoint[] {
    if (!Array.isArray(serie) || serie.length === 0)
        return [];
    const out: ExternalIndicatorSeriesPoint[] = [];
    for (const p of serie as MindicadorSeriePoint[]) {
...
function sortChronological(points: ExternalIndicatorSeriesPoint[]): ExternalIndicatorSeriesPoint[] {
    return [...points].sort((a, b) => new Date(a.fecha).getTime() - new Date(b.fecha).getTime());
}

function extractValueAndDate(item: MindicadorBlock | null | undefined): { value: number | null; fecha: string | null } {
...
@Injectable()
export class ExternalIndicatorsService {
    private async fetchSingleIndicator(
        key: IndicatorKey,
    ): Promise<{ value: number | null; fecha: string | null; unidad: string | null } | null> {
        ...
        for (let attempt = 1; attempt <= MAX_FETCH_RETRIES + 1; attempt++) {
            ...
            try {
                ...
                const body = (await res.json()) as MindicadorBlock;
                const parsed = extractValueAndDate(body);
                if (parsed.value == null && parsed.fecha == null) continue;
                return {
                    value: parsed.value,
                    fecha: parsed.fecha,
    ...
    private async getExternalIndicatorsFallbackByIndicator(): Promise<ExternalIndicatorsSnapshot | null> {
        ...
        const hasAny = Boolean(
            (dolar && (dolar.value != null || dolar.fecha != null)) ||
    ...
    private cache: { data: ExternalIndicatorsSnapshot; expiresAt: number } | null = null;
    private readonly seriesCache = new Map<string, { data: ExternalIndicatorsSeriesPayload; expiresAt: number }>();

    buildEmptyResponse(errorMessage: string): ExternalIndicatorsSnapshot {
        const empty = { value: null, fecha: null, unidad: null, error: true as const };
        return {
            dolar: { ...empty },
            uf: { ...empty },
            ipc: { ...empty },
    ...
    async getExternalIndicators(): Promise<ExternalIndicatorsSnapshot> {
        const now = Date.now();
        ...
        for (let attempt = 1; attempt <= MAX_FETCH_RETRIES + 1; attempt++) {
            ...
            try {
                ...
                const dolarItem = body?.dolar as MindicadorBlock | undefined;
                const ufItem = body?.uf as MindicadorBlock | undefined;
                const ipcItem = body?.ipc as MindicadorBlock | undefined;
                const dolar = extractValueAndDate(dolarItem);
                const uf = extractValueAndDate(ufItem);
                const ipc = extractValueAndDate(ipcItem);
                const data: ExternalIndicatorsSnapshot = {
                    dolar: {
                        value: dolar.value,
                        fecha: dolar.fecha,
    ...
    async fetchIndicatorByYear(code: string, year: number, signal?: AbortSignal): Promise<ExternalIndicatorSeriesPoint[]> {
        ...
        for (let attempt = 1; attempt <= MAX_FETCH_RETRIES + 1; attempt++) {
            ...
            try {
                ...
                const body = (await res.json()) as { serie?: unknown };
    ...
    async buildWeeklySeries(signal?: AbortSignal): Promise<ExternalIndicatorsSeriesPayload> {
        ...
        const takeLast7 = (arr: ExternalIndicatorSeriesPoint[]) => (arr.length <= 7 ? arr : arr.slice(-7));
    ...
    async buildMonthlySeries(signal?: AbortSignal): Promise<ExternalIndicatorsSeriesPayload> {
        const currentYear = new Date().getFullYear();
        const prevYear = currentYear - 1;
        const fetchTwoYears = async (code: string) => {
        ...
        const last12ByMonth = (points: ExternalIndicatorSeriesPoint[]) => {
            const byMonth = new Map<string, ExternalIndicatorSeriesPoint>();
    ...
    async buildYearlySeries(signal?: AbortSignal): Promise<ExternalIndicatorsSeriesPayload> {
        const currentYear = new Date().getFullYear();
        const years = [currentYear - 4, currentYear - 3, currentYear - 2, currentYear - 1, currentYear];
        const fetchOne = async (code: string) => {
            const all: ExternalIndicatorSeriesPoint[] = [];
    ...
    async getExternalIndicatorsSeries(period: "weekly" | "monthly" | "yearly"): Promise<ExternalIndicatorsSeriesPayload> {
        const now = Date.now();
