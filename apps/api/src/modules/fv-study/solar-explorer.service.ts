...
function normalizeExplorerApiKey(raw: string | undefined | null): string | null {
 ...
 */
function normalizeExplorerBaseUrl(raw: string | undefined | null): string | null {
    if (raw == null || typeof raw !== "string")
        return null;
...
function maskKeyHint(key: string | null): string {
...
function normalizePvwattsApiKey(raw: string | undefined | null): string | null {
    if (raw == null || typeof raw !== "string")
        return null;
    let k = raw.trim().replace(/\r?\n/g, "");
    if ((k.startsWith('"') && k.endsWith('"')) || (k.startsWith("'") && k.endsWith("'")))
...
function isPvwattsKeyPlaceholder(key: string): boolean {
    const u = key.trim();
    if (/^<[^>]+>$/.test(u))
        return true;
...
function shouldSkipSolarExplorerFirst(): boolean {
...
function getPvwattsConfig() {
    ...
    if (rejectedAsPlaceholder) {
        console.warn(
            "[PVWATTS] PVWATTS_API_KEY rechazada: parece placeholder de documentación (<API_KEY>, CAMBIAR_*, etc.). " +
    ...
    console.log(
        "[PVWATTS] PVWATTS_ENABLED =",
        enabled,
        "| PVWATTS_API_BASE_URL =",
        baseUrl ?? "(null)",
        "| respaldo operativo =",
        configured,
    );
...
type PvwattsMonthlyDerivation = "AC_MONTHLY" | "SOLRAD_PROPORTIONAL" | "ANNUAL_UNIFORM" | "INTERNAL_HSP_UNIFORM";

type PvwattsEstimateResult = {
    monthlyGeneration: Array<{ month: number; label: string; generationKwh: number }>;
    annualGenerationKwh: number;
    monthlyDerivation: PvwattsMonthlyDerivation;
};
...
function buildInternalHspEstimateFromContext(context: {
    ...
    hspDailyUsed?: number | null;
    performanceRatioUsed?: number | null;
}): PvwattsEstimateResult | null {
...
type PvwattsFailureDetail = {
    code: string;
    messageForUser: string;
    httpStatus?: number;
    nrelErrorCode?: string;
    nrelErrorMessage?: string;
    rawDetail?: string;
};

type PvwattsCallResult = { ok: true; estimate: PvwattsEstimateResult } | { ok: false; failure: PvwattsFailureDetail };

function isUsablePvwattsEstimate(e: PvwattsEstimateResult): boolean {
...
function tryRepairPvwattsAnnualFromMonthly(e: PvwattsEstimateResult): PvwattsEstimateResult | null {
    if (!Array.isArray(e.monthlyGeneration) || e.monthlyGeneration.length < 12)
        return null;
...
function nrelUserMessageForTopError(code: string, msg: string | undefined, usingDemoKey: boolean): string {
...
function normalizeAzimuthForNrel(deg: number): number {
    let a = deg % 360;
    if (a < 0)
        a += 360;
    if (a >= 360)
...
function monthlyKwhFromSolradAndAnnual(solradMonthly: number[], acAnnual: number): number[] {
...
function coerceNumberArray12(v: unknown): number[] | null {
...
function buildPvwattsEstimateFromOutputs(outputs: Record<string, unknown>): PvwattsEstimateResult | null {
...
type MinenergiaProxyPayloadCandidate = {
    label: string;
    body: Record<string, unknown>;
};

function logSolarFinalSuccess(provider: string, annual: number, monthlyCount: number) {
    console.log(`[SOLAR-FINAL] success provider=${provider} annual=${annual} monthlyCount=${monthlyCount}`);
}

function logSolarFinalFailure(rootProvider: string, reason: string, failedProvider?: string) {
...
function isMinenergiaOperational500Fail(f: { httpStatus?: number; reason?: string; bodyPreview?: string }): boolean {
    if (f.httpStatus !== 500)
        return false;
...
@Injectable()
export class SolarExplorerService {
    ...
    async requestExternalEstimate(context) {
        ...
        let minenergiaFailure: MinenergiaErr | null = null;
        ...
        if (config.providerConfigured && !skipExplorer) {
            ...
            if (explorerResult.ok) {
                ...
                logSolarFinalSuccess(
                    "EXPLORADOR_SOLAR",
                    explorerResult.annualGenerationKwh,
                    explorerResult.monthlyGeneration.length,
                );
                return {
                    ...base,
        ...
        let pvwattsFailure: PvwattsFailureDetail | null = null;
        if (pvwatts.configured) {
            ...
            if (pvCall.ok === false) {
                ...
                if (!isUsablePvwattsEstimate(pvResult)) {
                    console.warn(
                        "[SOLAR-EXPLORER] PVWatts: respuesta descartada (anual no finito o serie mensual incompleta/inválida). annual =",
                        pvResult.annualGenerationKwh,
                        "| meses =",
                        pvResult.monthlyGeneration?.length,
                    );
                    pvwattsFailure = {
                        code: "PVWATTS_INVALID_SERIES",
                        messageForUser:
                            "PVWatts respondió pero la serie de generación no es usable (valores no finitos o menos de 12 meses). Se intentará respaldo interno si el estudio tiene HSP/PR.",
                    };
                }
                else {
        ...
        const tail = (() => {
            if (pvwattsFailure) {
                return {
                    ...
                    failedProvider: "PVWATTS_V8" as const,
                };
            }
            if (!pvwatts.configured) {
                return {
                    message:
                        "No hay estimación disponible: PVWatts no está configurado en el servidor (habilitar servicio, URL y clave) y el estudio no tiene HSP ni factor de rendimiento (PR) válidos para calcular en interno.",
            ...
            if (!config.providerConfigured && !skipExplorer) {
                return {
                    message:
                        "No hay estimación disponible: Explorador Solar no está configurado, PVWatts no pudo completar el cálculo y el estudio no tiene HSP/PR para un respaldo interno.",
            ...
            return {
                message:
                    "No se obtuvo estimación: PVWatts no devolvió una serie mensual válida y el estudio no tiene HSP/PR para calcular en interno. Si el problema persiste, revise la configuración de PVWatts con su administrador.",
    ...
    async callMinenergiaProxy(context): Promise<MinenergiaOk | MinenergiaErr> {
         ...
         */
        const payloadCandidates: MinenergiaProxyPayloadCandidate[] = [
        ...
        let postResult: Awaited<ReturnType<SolarExplorerService["postToProxy"]> > | null = null;
        let lastPostFailure: { reason: string; httpStatus?: number; bodyPreview?: string } | null = null;
        for (let i = 0; i < payloadCandidates.length; i++) {
            ...
            if (i < payloadCandidates.length - 1) {
                console.warn(
                    "[SOLAR-EXPLORER] 500 OperationalError en proxy; probando variante de payload más conservadora.",
                );
            }
        }
    ...
    csvDownloadAuthHeaders(apiBase: string, authorizationValue: string, csvUrl: string): Record<string, string> | undefined {
        try {
    ...
    async postToProxy(proxyUrl: string, headers: Record<string, string>, body: object): Promise<
        | { ok: true; csvUrl: string }
        | { ok: false; reason: string; httpStatus?: number; bodyPreview?: string }
    > {
    ...
    async downloadCsv(csvUrl: string, extraHeaders?: Record<string, string>) {
        const controller = new AbortController();
        const timeout = setTimeout(() => controller.abort(), SOLAR_EXPLORER_TIMEOUT_MS);
        try {
    ...
    async callPvwattsV8(context, usingDemoKey: boolean): Promise<PvwattsCallResult> {
        ...
        try {
            ...
            const topErr = data.error as { code?: string; message?: string } | undefined;
            ...
            if (!built) {
                console.warn(
                    "[PVWATTS] monthly derivation failed | outputs (recorte) =",
                ...
                return {
                    ...
                    failure: {
                        ...
                        messageForUser:
                            "PVWatts devolvió outputs sin ac_monthly (12), ni ac_annual + solrad_monthly, ni ac_annual usable para reparto.",
                        httpStatus: res.status,
