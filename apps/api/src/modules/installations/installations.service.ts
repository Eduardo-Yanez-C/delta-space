...
import type { ActivateInstallationDto } from "./dto/activate-installation.dto";

@Injectable()
export class InstallationsService {
  constructor(private readonly prisma: PrismaService) {}

  async activate(dto: ActivateInstallationDto) {
    const code =
      typeof dto.activationCode === "string" ? dto.activationCode.trim() : "";
    if (!code) {
      throw new BadRequestException("activationCode es obligatorio");
    }
    const activationCode = await this.prisma.activationCode.findUnique({
      where: { code },
    });
    if (!activationCode) {
      throw new BadRequestException("Código de activación no válido");
    }
    ...
    if (activeCount >= activationCode.maxActivations) {
      throw new BadRequestException(
        "Se ha alcanzado el número máximo de instalaciones para este código",
      );
    }
    const installationToken = randomBytes(32).toString("hex");
    const deviceName =
      typeof dto.deviceName === "string" && dto.deviceName.trim() !== ""
        ? dto.deviceName.trim()
        : null;
    const appVersion =
      typeof dto.appVersion === "string" && dto.appVersion.trim() !== ""
        ? dto.appVersion.trim()
        : null;
    const machineFingerprint =
      typeof dto.machineFingerprint === "string" &&
  ...
  async validate(installationId: string, installationToken: string) {
    const id =
      typeof installationId === "string" ? installationId.trim() : "";
    const token =
      typeof installationToken === "string" ? installationToken.trim() : "";
    if (!id || !token) {
      return {
        valid: false,
        active: false,
  ...
  async revoke(id: string, note: string | undefined) {
    const installation = await this.prisma.installation.findUnique({
      where: { id },
