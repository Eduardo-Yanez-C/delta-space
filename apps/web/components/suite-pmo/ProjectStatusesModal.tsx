...
const SECTIONS: {
  ...
  { category: "not_started", title: "No iniciado", hint: "Pendiente / cola" },
  { category: "active", title: "En curso", hint: "Trabajo activo" },
  { category: "done", title: "Hecho", hint: "Cerrado / completado" },
];
...
export function ProjectStatusesModal({
  open,
  onClose,
  projectId,
  initialRaw,
  ...
  const [cfg, setCfg] = useState<TaskStatusConfig>(DEFAULT_TASK_STATUS_CONFIG);
  ...
  useEffect(() => {
    if (!open) return;
    setCfg(normalizeTaskStatusConfig(initialRaw));
    setErr(null);
  }, [open, initialRaw]);
  ...
  const save = useCallback(async () => {
    ...
    try {
      await updateSuiteProject(projectId, {
        taskStatusConfig: cfg as unknown as Record<string, unknown>,
      });
      onSaved();
      onClose();
    } catch (e) {
      ...
      <div className="fixed left-1/2 top-1/2 z-[75] w-[min(100vw-24px,520px)] max-h-[min(90vh,720px)] -translate-x-1/2 -translate-y-1/2 overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-2xl dark:border-slate-600 dark:bg-slate-950">
        <div className="flex items-start justify-between gap-3 border-b border-slate-200 px-4 py-3 dark:border-slate-700">
          <div>
            ...
            <p className="mt-0.5 text-xs text-slate-500">
              Define el flujo de estados de las tareas. Los valores se guardan
              en el proyecto.
            </p>
          </div>
          <button
        ...
        <div className="flex flex-wrap items-center justify-end gap-2 border-t border-slate-200 bg-slate-50 px-4 py-3 dark:border-slate-700 dark:bg-slate-900/80">
          ...
          <button
            ...
            disabled={saving}
            onClick={() => void save()}
            className="rounded-lg bg-sky-600 px-5 py-2 text-sm font-semibold text-white shadow-sm hover:bg-sky-500 disabled:opacity-50 dark:bg-sky-500"
          >
