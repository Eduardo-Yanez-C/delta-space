/**
 * Lógica alineada a la planilla «MRB Proyectos Gx VF»:
 * hojas BASE (P)(I), SEVERIDAD, EFECTIVIDAD CONTROL y estructura de MATRIZ DE RIESGOS.
 *
 * Copiado/adaptado desde el proyecto base para evitar dependencia workspace `mrb-matrix`.
 */
...
/** Fila p (1..5) × i (1..5): severidad inherente y valor (= P×I). */
...
export function inherentFromLabels(probabilityLabel: string, impactLabel: string): { severity: string; value: number } {
  const pv = lookupProbabilityValue(probabilityLabel);
  const iv = lookupImpactValue(impactLabel);
  if (!pv || !iv) return { severity: "—", value: 0 };
  return inherentFromValues(pv, iv);
}
...
/** Subconjunto de «EFECTIVIDAD CONTROL»: clave = periodicidad|oportunidad|automatización (normalizado). */
...
/** Completa cálculos MRB y devuelve objeto versionado para persistir en JSON. */
